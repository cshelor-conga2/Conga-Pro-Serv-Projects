...
crmc.require(["KendoEntry", "KendoPopup", "ListButton", "sfdc"], function(prompt, popup, navigate, sfdc) {
  ...
  var TEXT_BATCH_Update_OBJECT = "Batch Update Quote Lines";
  var TEXT_ERROR = "There was an error: ";
  var TEXT_MESSAGE = "Updated {0} Quote Lines, would you like to refresh the page?";
  var TEXT_BATCH_UPDATE_RESULTS = "Batch Update Results";
  var TEXT_YES = "Yes";
  var TEXT_NO = "No";

  crmc.addCustomAction({
    "itemID": "AG_BatchIncreaseQuoteLineUnitPrice",
    "isAvailable": function (context) {
      ...
      var isContext = context.objectDescribe.name == "SBQQ__QuoteLine__c";
    ...
    "click": function (context) {
      ...
      prompt.entry("Batch Update Quote Lines",
        ...
        function(selectedValues) {
          ...
          var data= sfdc.query("SELECT Id, SBQQ__NetPrice__c FROM SBQQ__QuoteLine__c WHERE Id IN('" + ids.join("','") + "')");
          for (var i = 0; i < data.length; i++) {
            var row = data[i];
            var record = new sforce.SObject("SBQQ__QuoteLine__c");
            record["SBQQ__NetPrice__c"] = row.SBQQ__NetPrice__c*(1+(DisPer/100));
            record["Id"] = row.Id;
            records.push(record);
          };
