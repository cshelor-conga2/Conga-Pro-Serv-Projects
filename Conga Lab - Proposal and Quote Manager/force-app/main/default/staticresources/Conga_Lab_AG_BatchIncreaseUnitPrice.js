...
crmc.require(["KendoEntry", "KendoPopup", "ListButton", "sfdc"], function(prompt, popup, navigate, sfdc) {
  ...
  var TEXT_BATCH_Update_OBJECT = "Batch Update Products";
  var TEXT_ERROR = "There was an error: ";
  var TEXT_MESSAGE = "Updated {0} Products, would you like to refresh the page?";
  var TEXT_BATCH_UPDATE_RESULTS = "Batch Update Results";
  var TEXT_YES = "Yes";
  var TEXT_NO = "No";

  crmc.addCustomAction({
    "itemID": "AG_BatchIncreaseUnitPrice",
    "isAvailable": function (context) {
      ...
      var isContext = context.objectDescribe.name == "OpportunityLineItem"
    ...
    "click": function (context) {
      ...
      prompt.entry("Batch Update Products",
        ...
        function(selectedValues) {
          ...
          var data= sfdc.query("SELECT Id, UnitPrice FROM OpportunityLineItem WHERE Id IN('" + ids.join("','") + "')");
          for (var i = 0; i < data.length; i++) {
            var row = data[i];
            var record = new sforce.SObject("OpportunityLineItem");
            record["UnitPrice"] = row.UnitPrice*(1+(DisPer/100));
            record["Id"] = row.Id;
            records.push(record);
          };
